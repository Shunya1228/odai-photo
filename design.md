...
- ログインなし
- サーバーへのデータ保存なし(履歴は残らない)
- 進行中のお題のみ端末内に一時保存し、アプリを開き直したときに復元
...
PWA（Progressive Web App）は、Web サイトをスマートフォンのホーム画面に追加し、通常のアプリに近い形で起動できる仕組み。アプリストアを介さず URL から利用でき、全画面表示や静的ファイルのキャッシュなどに対応できる。

### なぜ PWA なのか

このアプリは、外出中にお題を確認し、スマートフォンで写真を撮って遊ぶことを想定している。PWA を採用する理由は次の3点。

- URL を開くだけで試せ、気に入ればホーム画面からすぐ起動できる
- アプリストアへの公開や審査なしで、MVP を素早く改善・配信できる
- ネイティブアプリを作る前に、利用ニーズを低コストで検証できる

PWA によって特別なカメラ機能が増えるわけではない。また、お題生成と採点には通信が必要なため、ゲーム全体のオフライン対応は目的にしない。キャッシュの方針は §8.3 に記載する。
...
お題を確認してから撮れるものを探す間に、ユーザーがアプリをバックグラウンドへ移動したり、OS によってアプリのプロセスが終了されたりする可能性がある。`useState` だけでは再読み込み時に状態が失われるため、**進行中のお題は `localStorage` に保存して復元する**。
...
- お題の生成に成功した時点で `localStorage` に保存
- アプリ起動時に保存済みのお題があれば、`has-prompt` として復元
- 「お題を引き直す」場合は、新しいお題で上書き
- 採点に成功したとき、または「もう一回」を押したときに削除
- 壊れたデータや想定外のバージョンは破棄して `idle` から開始

写真の `File` / Blob は `localStorage` に保存しない。容量が大きく、端末内に写真を意図せず残すことになるためである。通常、アプリからカメラを開いて戻るだけなら画面の状態は維持されるが、その間にアプリが終了・再読み込みされた場合は、お題だけを復元して写真は再撮影または再選択してもらう。

これは「過去のお題を残す履歴機能」ではなく、現在進行中の1件を復元するための端末内データである。また、Service Worker のキャッシュとは別の仕組みとして扱う。
...
- DB なし、Storage なし
- 進行中のお題1件だけを端末の `localStorage` に保存し、再起動時に復元
- 写真・採点結果・過去のお題は保存しない
...
Phase 6 で Next.js 16 との互換性を確認し、`next-pwa` または手動実装を選択する。

- JavaScript、CSS、アイコンなどの静的アセットをキャッシュし、2回目以降の画面表示を速くする
- お題生成、写真送信、採点に使う Server Action / API 通信はネットワークのみとし、キャッシュ対象から明示的に除外する
- Gemini API が利用できないオフライン状態では、画面を開けてもお題生成と採点はできない旨を表示する

Service Worker のキャッシュはアプリを構成するファイルの再利用が目的であり、進行中のお題を保持する仕組みではない。進行中のお題の復元には `localStorage` を使用する(§4.3)。
...
- 進行中のお題を `localStorage` に保存・復元・削除する処理(§4.3)
- ローディング UI
...
- アイコン作成(暫定)
- Next.js 16 との互換性を確認し、`next-pwa` または手動 Service Worker を導入
- 静的アセットのみをキャッシュし、Server Action / API 通信を対象外にする
- iOS 向けメタタグ
- ホーム画面追加促進バナー
